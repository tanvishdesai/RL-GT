...
def train_selfplay_pennies(env: MatchingPenniesEnvironment, episodes: int, input_dim: int, output_dim: int, device: torch.device) -> PPOAgent:
    ...
    for ep in range(episodes):
        try:
            ...
            for _ in range(1):
                ...
                b = opponent.act(s, opponent_history=hist)
                ns, rewards, _, _ = env.step([a, b]); r = rewards[0]
                ...
                if torch.isnan(loss) or torch.isinf(loss):
                    print(f"Warning: Invalid loss at episode {ep}, skipping update")
                    continue
                
                opt.zero_grad(); loss.backward(); opt.step(); s = ns
                
        except Exception as e:
            print(f"Error in self-play episode {ep}: {e}")
            continue
            
        if (ep + 1) % 25 == 0: 
            opponent.add_snapshot(agent)
        if (ep + 1) % 1000 == 0:
            print(f"Self-Play progress: {ep+1}/{episodes} episodes completed")
            
    print("Training finished for Self-Play (Matching Pennies).")
    return agent
...
def train_psro_pennies(gauntlet: "EnhancedGauntletBenchmark", iterations: int = 5, episodes_per_iter: int = 100) -> PSROPolicy:
    ...
    for i in range(iterations):
        print(f"[PSRO-Pennies] Iteration {i+1}/{iterations}")
...
if __name__ == "__main__":
    ...
    psro_policy = train_psro_pennies(gauntlet, iterations=iterations, episodes_per_iter=episodes_per_iter)
    ...
    print(f"\n--- Training Pennies Self-Play ({time_budget_seconds}s, {selfplay_episodes} episodes) ---")
    _t0 = time.time()
    sp_agent = train_selfplay_pennies(env, episodes=selfplay_episodes, input_dim=input_dim, output_dim=output_dim, device=torch.device('cpu'))
