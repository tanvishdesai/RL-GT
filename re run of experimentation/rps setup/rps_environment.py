%%writefile rps_training_and_evaluation.py
...
# Import the unified PRPO classes
...
class RPSEnvironment:
    ...
    def get_legal_actions(self, player_id: int = 0) -> List[int]:
        """Return all legal actions for RPS (all actions are always legal)."""
...
def train_selfplay_rps(env, time_budget_seconds: float, input_dim: int, output_dim: int, device: torch.device) -> PPOAgent:
    """
    Train Self-Play for RPS using actual time budget instead of fixed episodes.
    """
    print(f"[SelfPlay-RPS] Training for {time_budget_seconds} seconds...")
    ...
    while (time.time() - start_time) < time_budget_seconds:
        ...
        if (current_time - last_log_time) >= 10.0:
            ...
            print(f"    [SelfPlay-RPS] Time {elapsed_time:.1f}s: Episodes {episode_count}")
            last_log_time = current_time

    final_time = time.time() - start_time
    print(f"[SelfPlay-RPS] Training completed in {final_time:.1f}s with {episode_count} episodes")
    return agent
...
def train_psro(env, gauntlet: "EnhancedGauntletBenchmark", time_budget_seconds: float, episodes_per_iter: int = 100) -> PSROPolicy:
    """
    Train PSRO for RPS using actual time budget instead of fixed iterations.
    """
    print(f"[PSRO-RPS] Training for {time_budget_seconds} seconds...")
    start_time = time.time()
    ...
    while (time.time() - start_time) < time_budget_seconds:
        ...
        print(f"[PSRO-RPS] Iteration {iteration_count + 1} (Elapsed: {elapsed_time:.1f}s)")
        ...
        if iteration_count >= 20:  # Max reasonable iterations
            print(f"[PSRO-RPS] Reached maximum iterations ({iteration_count}), stopping.")
            break

    final_time = time.time() - start_time
    print(f"[PSRO-RPS] Training completed in {final_time:.1f}s with {iteration_count} iterations")
    
    if not population:
        # Fallback: train at least one best response if no time was sufficient
        print("[PSRO-RPS] Warning: No iterations completed, training one best response")
        br_policy = train_best_response_dqn(env_train, opponent_pool, episodes_per_iter, input_dim, output_dim)
        population.append(copy.deepcopy(br_policy))

    meta = _compute_meta_strategy(population, opponent_pool, env_train)
    print(f"[PSRO-RPS] Learned meta-strategy over {len(population)} policies: {np.round(meta, 3)}")
    print(f"[PSRO-RPS] PSRO training completed successfully!")
...
if __name__ == "__main__":
    ...
    psro_policy = train_psro(training_env, gauntlet_psro, time_budget_seconds=time_budget_seconds, episodes_per_iter=100)
    ...
    _t0 = time.time()
    sp_agent = train_selfplay_rps(training_env, time_budget_seconds=time_budget_seconds, input_dim=input_dim, output_dim=output_dim, device=torch.device('cpu'))
