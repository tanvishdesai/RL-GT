    ...
    def update_policy(self):
        ...
        for _ in range(self.k_epochs):
            ...
            if self.lambda_target > 0 and self.target_policy_state_dict is not None:
                # Avoid regularizing against self if this agent is the target
                ...
                if not is_self:
                    ...
                    # D_KL(target || current) pushes current policy to match target
...
def run_single_experiment(algorithm_name, time_budget_seconds=120, seed=42):
    ...
    if algorithm_name in ["Standard PPO", "Self-Play"]:
        ...
        prpo_system = UnifiedPRPO(
            ...
            lambda_exploit_base=0.5,  # Penalty for being exploitable
            lambda_target=0.2,        # Penalty for deviating from best-in-population
...
if __name__ == "__main__":
    # The main execution block can be updated to test the new components.
    # For a full run, all functions would be called.
    # 1. Baseline comparison
    print("\n📊 1. BASELINE COMPARISON")
    run_comparison(num_seeds=1, time_budget_per_alg_seconds=1500)
    
    # 2. Ablation study
    print("\n📊 2. ABLATION STUDY")
    run_ablation_study(num_seeds=1, time_budget_per_alg_seconds=1500)
    
    # 3. Hyperparameter sensitivity
    print("\n📊 3. HYPERPARAMETER SENSITIVITY")
    run_hyperparameter_sensitivity(num_seeds=1, time_budget_per_alg_seconds=1500)
