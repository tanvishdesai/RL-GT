...
def train_psro_stag_hunt(gauntlet: "EnhancedGauntletBenchmark", iterations: int = 5, episodes_per_iter: int = 1000) -> PSROPolicy:
    """Train PSRO on Stag Hunt."""
    print(f"Training PSRO for {iterations} iterations...")
    ...
    for i in range(iterations):
        print(f"PSRO iteration {i+1}/{iterations}")
        ...
        print(f"Added agent {i+1} to population. Population size: {len(population)}")
...
if __name__ == "__main__":
    ...
    print(f"\n--- Training PSRO (5 iterations with time budget) ---")
    ...
    psro_policy = train_psro_stag_hunt(gauntlet, iterations=5, episodes_per_iter=episodes_per_iter)
    ...
    selfplay_agent = train_stag_hunt_agent(copy.deepcopy(dqn_agent), env, copy.deepcopy(dqn_agent), num_episodes=sp_episodes)
