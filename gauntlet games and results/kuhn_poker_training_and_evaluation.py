...
def train_selfplay_kuhn(env: KuhnPokerEnvironment, episodes: int, input_dim: int, output_dim: int, device: torch.device) -> PPOAgent:
    ...
    for _ in range(episodes):
        ...
        if random.random() < 0.2: opponent.add_snapshot(agent)
    print("Training finished for Self-Play (Kuhn).")
    return agent
...
def train_psro_kuhn(gauntlet: "EnhancedGauntletBenchmark", iterations: int = 5, episodes_per_iter: int = 1000) -> PSROPolicy:
    ...
    for i in range(iterations):
        print(f"[PSRO-Kuhn] Iteration {i+1}/{iterations}")
...
if __name__ == "__main__":
    ...
    print(f"\n--- Training Kuhn PSRO ({PSRO_ITERATIONS} iterations with time budget) ---")
    _t0 = time.time()
    # Adjust episodes per iteration based on time budget
    episodes_per_iter = max(100, int(TIME_BUDGET_SECONDS / PSRO_ITERATIONS))
    psro_policy = train_psro_kuhn(gauntlet, iterations=PSRO_ITERATIONS, episodes_per_iter=episodes_per_iter)
    ...
    _t0 = time.time()
    # Use time budget for Self-Play
    sp_episodes = int(TIME_BUDGET_SECONDS * 100)  # Approximate episodes based on time budget
    sp_agent = train_selfplay_kuhn(env, episodes=sp_episodes, input_dim=input_dim, output_dim=output_dim, device=torch.device('cpu'))
